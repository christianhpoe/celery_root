...
from celery_cnc.config import MAX_PORT, CeleryCnCConfig, FrontendConfig, get_settings
...
def _resolve_worker_paths(values: Sequence[str]) -> list[str]:
    ...
    if paths:
        return paths
    env_raw = os.getenv("CELERY_CNC_WORKERS")
    if not env_raw:
        return []
...
def _apply_worker_paths(config: CeleryCnCConfig, paths: Sequence[str]) -> CeleryCnCConfig:
    if not paths:
        return config
    seen: set[str] = set()
...
def _run_cnc(apps: Sequence[Celery], config: CeleryCnCConfig) -> None:
    if not apps:
        message = "No Celery app configured. Use -A/--app, a worker path argument, or set CELERY_CNC_WORKERS."
        raise click.UsageError(message)
    cnc = CeleryCnC(*apps, config=config)
    cnc.run()
...
@click.pass_context
def cnc(ctx: click.Context, host: str | None, port: int | None, *, debug: bool | None) -> None:
    ...
    if app is None:
        ...
        loaded_apps = _load_apps(paths)
        if not loaded_apps:
            message = "Celery app not provided. Use `celery -A module:app cnc` or set CELERY_CNC_WORKERS."
            raise click.UsageError(message)
        _run_cnc(loaded_apps, config)
        return
    _run_cnc((app,), config)

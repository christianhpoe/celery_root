...
from celery_cnc.web.views import dashboard as dashboard_views
...
def _build_auth(config: CeleryCnCConfig) -> StaticTokenVerifier | None:
    if not config.mcp_auth_key:
        return None
    return StaticTokenVerifier(tokens={config.mcp_auth_key: {"client_id": "celery_cnc_mcp"}})
...
def create_mcp_server() -> FastMCP:
    ...
    mcp = FastMCP(name="Celery CnC MCP", auth=_build_auth(config))
...
def create_asgi_app() -> ASGIApp:
    ...
    base_path = _normalize_path(config.mcp_path)

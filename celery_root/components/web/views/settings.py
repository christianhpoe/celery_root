...
def settings_page(request: HttpRequest) -> HttpResponse:
    ...
    host = getattr(settings, "CELERY_ROOT_MCP_HOST", "127.0.0.1")
    port = getattr(settings, "CELERY_ROOT_MCP_PORT", 9100)
    path = getattr(settings, "CELERY_ROOT_MCP_PATH", "/mcp/")
    ...
    codex_command = f'codex mcp add celery_root --url "{mcp_url}?token=<YOUR_MCP_AUTH_KEY>"'
    claude_command = (
        f'claude mcp add --transport http celery_root {mcp_url} --header "Authorization: Bearer <YOUR_MCP_AUTH_KEY>"'
    )
    return render(
        request,
        "settings.html",

...
// Load worker detail content asynchronously for faster navigation.
(function () {
  ...
  function init() {
    const container = document.querySelector("[data-worker-fragment]");
    if (!container) {
      return;
    }
    const url = container.dataset.workerFragment;
    if (!url) {
      return;
    }
    const loading = document.querySelector("[data-worker-loading]");
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error("Failed to load worker detail");
        }
      ...
      .catch(() => {
        ...
        container.innerHTML =
          "<section class=\"detail-card\"><h3>Worker detail</h3><p>Unable to load worker data.</p></section>";
      });
  }
